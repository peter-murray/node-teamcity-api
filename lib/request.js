...
HttpRequest.prototype.execute = function (api, parameters) {
  ...
  promise = axios.request(api.getRequest(parameters))
    //TODO clean up error messages in the generated errors if need be
    .then(function(res) {
      return res.data;
      // if (res.status === api.getSuccessCode()) {
      //   return res.data;
      // } else {
      //   if (res.data) {
      //     console.error("******")
      //     throw new Error(util.format("Unexpected Status Code, %s\n%s", res.status, res.data));
      //   } else {
      //     console.error("******")
      //     throw new Error(util.format("Unexpected Status Code, %s, expected %s", res.status, api.getSuccessCode()));
      //   }
      // }
    })
...
function generateRequestConfig(config) {
  ...
  requestConfig.timeout = 2000;//TODO make configurable
