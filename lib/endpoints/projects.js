...
function createProjectPayload(data) {
...
function copyProjectPayload(data) {
  var body = util.format(
      '<newProjectDescription name="%s" id="%s" copyAllAssociatedSettings="%s">' +
      '<parentProject locator="id:%s"/><sourceProject locator="id:%s"/></newProjectDescription>',
      data.name,
      data.id,
      data.copyAllSettings ? "true" : "false",
      data.parentId,
      data.sourceId
  );
